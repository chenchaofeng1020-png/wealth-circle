  ...
  <aside class="rightbar-container" ref="rightbarRef">
    ...
    <div class="widget-card circle-profile-card">
      ...
      <div class="circle-card-content">
        <div class="circle-days-badge">创建 512 天</div>
        <h3 class="circle-brand-name">财不外露</h3>
        <p class="circle-intro-text">
          这里是专注全球资产配置与家庭理财实践的成长基地，由一群深耕资管多年的老伙计打造。我们不堆{{ isIntroExpanded ? '砌晦涩概念，只输出穿越周期的实战框架与底仓逻辑。提供全方位大类资产配置、公募基金优选、固收与海外宽基配置方案，陪伴每一位圈友稳健成长。' : '...' }}
        </p>
        <button class="btn-expand-intro" @click="isIntroExpanded = !isIntroExpanded">
          {{ isIntroExpanded ? '收起' : '展开' }}
        </button>

        <!-- Community Members Entrance (查看社区成员入口) -->
        <div 
        ...
        >
          <div class="members-entry-left">
            ...
            <div class="members-count-desc">
              ...
              <span class="count-unit">位圈友</span>
            </div>
          </div>
          <div class="members-entry-right">
            <span class="entry-link-text">全部成员</span>
    ...
    <div class="widget-card ask-experts-card">
      ...
      <div class="experts-list">
        ...
        <div class="expert-item">
          ...
          <button class="btn-ask-circle" title="向 Rich 提问" @click="handleAsk('Rich')">
            问
          </button>
        </div>
        ...
        <div class="expert-item">
          ...
          <button class="btn-ask-circle" title="向 John 提问" @click="handleAsk('John')">
            问
          </button>
        </div>
        ...
        <div class="expert-item">
          ...
          <button class="btn-ask-circle" title="向 Jerry 提问" @click="handleAsk('Jerry')">
            问
          </button>
        </div>
      </div>
...
const handleAsk = (expertName: string) => {
  if (appState.user.role === 'guest') {
    actions.requireVip('开通会员后即可向合伙人/专享专家提问');
    return;
  }
  actions.showToast(`已向 ${expertName} 发起提问，专享专家将即时解答`);
};
