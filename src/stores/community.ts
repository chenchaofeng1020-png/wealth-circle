...
export const appState = reactive({
  // 顶层流程状态：'landing' (介绍页) | 'paywall' (开通会员页) | 'community' (主社区)
  ...
  // 视角切换：'founder' (圈主管理员视角) | 'vip' (普通会员视角) | 'guest' (注册未开通会员视角)
...
// 计算属性：当前用户是否具备管理权限（圈主或管理员或助教）
export const isManager = computed(() => {
  return appState.user.role === 'founder' || appState.user.role === 'admin' || appState.user.role === 'assistant';
});
...
export const actions = {
  ...
  submitAuth(phone: string = '13800008888', nickname?: string) {
    ...
    appState.user = {
      ...
      name: nickname?.trim() || ('新晋圈友_' + (phone.length >= 4 ? phone.slice(-4) : '8888')),
      avatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80',
      role: 'guest',
      roleTitle: '已注册 (未开通VIP)',
      bio: '新加入「财不外露」社区，正在开启资产配置进阶之旅。',
    ...
    appState.currentView = 'feed';
    actions.showToast('🎉 注册成功！欢迎加入「财不外露」私享社区');
  },
  ...
  skipToCommunity() {
    ...
    appState.isVipMember = true;
    appState.appFlowState = 'community';
    actions.showToast('已直接进入「财不外露」私享社区');
  },
  ...
  publishPost(payload: { title: string; content: string; tags: string[]; stocks?: string[]; isVipOnly: boolean }) {
    const newPost: Post = {
      ...
      author: {
        ...
        badge: appState.user.role === 'founder' ? '圈主动态' : 'VIP动态',
      },
      isPinned: false,
    ...
    actions.showToast('主题动态发布成功！已同步至社区信息流');
  },
  ...
  updateMemberRole(memberId: string, newRole: UserRole) {
    ...
    if (m) {
      m.role = newRole;
      actions.showToast(`已将 ${m.name} 的角色调整为：${newRole === 'assistant' ? '特邀助教' : newRole === 'vip' ? 'VIP会员' : '普通用户'}`);
    }
  },
  ...
  removeMember(memberId: string) {
    ...
    if (idx !== -1) {
      ...
      actions.showToast(`已将成员 ${name} 移出圈子`);
    }
  },
  ...
  createLiveSession(live: Partial<LiveSession>) {
    const newLive: LiveSession = {
      ...
      description: live.description || '社区专属直播，聚焦大类资产与行业最新动态',
